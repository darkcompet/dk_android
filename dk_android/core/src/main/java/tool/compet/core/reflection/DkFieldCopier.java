...
import tool.compet.core.DkCaller3;
...
public class DkFieldCopier {
	 ...
	 * @param dst               To object.
	 * @param annoClass         Annotation which be stickied to each copy-target field.
	 * @param upSuper           True to include super fields, False to exclude super fields.
	 * @param filterCallback	Filter before perform copy, return true to ignore copy, false to allow copy.
	 *                          This callback give back 3 params: field_name, src_value, dst_value to caller.
	 */
	public static void copy(Object src, Object dst, Class<? extends Annotation> annoClass, boolean upSuper,
		@Nullable DkCaller3<String, Object, Object, Boolean> filterCallback) {
		...
		if (N != dstFieldMap.size()) {
			DkLogs.warning(DkFieldCopier.class, "Different number of copy fields between %s(%d) vs %s(%d)",
		...
		for (int index = N - 1; index >= 0; --index) {
			...
			if (dstField != null) {
				try {
					...
					DkUtils.complainAt(DkFieldCopier.class, "Could not copy field: %s.%s -> %s.%s",
