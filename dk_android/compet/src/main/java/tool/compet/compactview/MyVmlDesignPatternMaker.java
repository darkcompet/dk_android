...
class MyVmlDesignPatternMaker {
	 ...
	 */
	static <L extends DkCompactLogic, D> void make(DkCompactView view, FragmentActivity host, @Nullable Bundle savedInstanceState) {
		...
		if (viewLogics.size() == 0 && viewModels.size() == 0) {
			DkLogcats.warning(MyVmlDesignPatternMaker.class, "Not found Logic and Model for view `%s` -> Maybe configuration missing?", viewClass.getName());
