...
import tool.compet.chips.DropdownChipLayouter.PermissionRequestDismissedListener;
import tool.compet.chips.RecipientAlternatesAdapter.RecipientMatchCallback;
import tool.compet.chips.recipientchip.DrawableRecipientChip;
import tool.compet.chips.recipientchip.InvisibleRecipientChip;
import tool.compet.chips.recipientchip.ReplacementDrawableSpan;
import tool.compet.chips.recipientchip.VisibleRecipientChip;

/**
 * RecipientEditTextView is an auto complete text view for use with applications
 * that use the new Chips UI for addressing a message to recipients.
 */
public class RecipientEditTextView extends AppCompatMultiAutoCompleteTextView implements OnItemClickListener, Callback, RecipientAlternatesAdapter.OnCheckedItemChangedListener, GestureDetector.OnGestureListener, TextView.OnEditorActionListener, DropdownChipLayouter.ChipDeleteListener, PermissionRequestDismissedListener {
	 ...
	 */
	public void setUntrustedAddressWarning(Set<String> untrustedAddresses, Bitmap warningIcon, int warningIconHeight, String warningTextTemplate, String warningTitle) {
	...
	public void setPermissionsRequestItemClickedListener(PermissionsRequestItemClickedListener listener) {
	...
	public void onRestoreInstanceState(Parcelable state) {
		...
		if (!TextUtils.isEmpty(getText())) {
			...
			super.onRestoreInstanceState(savedInstanceState.getParcelable(STATE_TEXT_VIEW));
		}
		String savedWarningText = savedInstanceState.getString(STATE_CURRENT_WARNING_TEXT);
			...
			public void onChanged(List<RecipientEntry> entries) {
				...
				if (entries != null && entries.size() > 0) {
					...
					if (mCurrentSuggestionCount == 0) {
						...
						announceForAccessibilityCompat(getSuggestionDropdownOpenedVerbalization(suggestionCount));
					}
				}
				...
				if ((entries == null || entries.size() == 0)
					...
					// available.
					&& getText().length() > 0) {
					announceForAccessibilityCompat(getResources().getString(R.string.accessbility_suggestion_dropdown_closed));
				}

				if ((entries != null) && (entries.size() == 1) && (entries.get(0).getEntryType() == RecipientEntry.ENTRY_TYPE_PERMISSION_REQUEST)) {
	...
	private ChipBitmapContainer createChipBitmap(RecipientEntry contact, TextPaint paint, Drawable overrideBackgroundDrawable, int backgroundColor) {
		...
		if (contact.getIndicatorIconId() != 0) {
			...
			indicatorIcon.setBounds(0, 0, indicatorIcon.getIntrinsicWidth(), indicatorIcon.getIntrinsicHeight());
		...
		int iconWidth = displayIcon ? height - backgroundPadding.top - backgroundPadding.bottom : 0;

		final boolean shouldDisplayWarningIcon = mUntrustedAddresses.contains(contact.getDestination());
		...
		CharSequence ellipsizedText = ellipsizeText(createChipDisplayText(contact), paint, calculateAvailableWidth() - iconWidth - warningIconWidth - warningIconEndMargin - widths[0] - backgroundPadding.left - backgroundPadding.right - indicatorPadding);
		...
		int width = Math.max(iconWidth * 2, textWidth + startPadding + mChipTextEndPadding + iconWidth + (int) warningIconWidth + (int) warningIconEndMargin + backgroundPadding.left + backgroundPadding.right + indicatorPadding);
		...
		int textX = shouldPositionAvatarOnRight() ? mChipTextEndPadding + backgroundPadding.left + indicatorPadding + (int) warningIconWidth + (int) warningIconEndMargin : width - backgroundPadding.right - mChipTextEndPadding - textWidth - indicatorPadding - (int) warningIconWidth - (int) warningIconEndMargin;
		canvas.drawText(ellipsizedText, 0, ellipsizedText.length(), textX, getTextYOffset(height), paint);

		if (indicatorIcon != null) {
			int indicatorX = shouldPositionAvatarOnRight() ? backgroundPadding.left + mChipTextEndPadding : width - backgroundPadding.right - indicatorIcon.getBounds().width() - mChipTextEndPadding;
		...
		final int iconX = shouldPositionAvatarOnRight() ? width - backgroundPadding.right - iconWidth : backgroundPadding.left;
		result.left = iconX;
		...
		final float warningIconX = shouldPositionAvatarOnRight() ? backgroundPadding.left + warningIconEndMargin : width - backgroundPadding.right - warningIconWidth - warningIconEndMargin;
	...
	private RectF drawWarningIcon(ChipBitmapContainer bitMapResult) {
		...
		final RectF dst = new RectF(bitMapResult.warningIconLeft, bitMapResult.warningIconTop, bitMapResult.warningIconRight, bitMapResult.warningIconBottom);
	...
	private void loadAvatarIcon(final RecipientEntry contact, final ChipBitmapContainer bitmapContainer) {
		...
		boolean drawPhotos = isPhoneQuery() ? contactId != RecipientEntry.INVALID_CONTACT : (contactId != RecipientEntry.INVALID_CONTACT && contactId != RecipientEntry.GENERATED_CONTACT);
			...
			if (origPhotoBytes == null) {
				...
				getAdapter().fetchPhoto(contact, new PhotoManager.PhotoManagerCallback() {
					...
					public void onPhotoBytesAsynchronouslyPopulated() {
						...
						final Bitmap icon = BitmapFactory.decodeByteArray(loadedPhotoBytes, 0, loadedPhotoBytes.length);
				...
				final Bitmap icon = BitmapFactory.decodeByteArray(origPhotoBytes, 0, origPhotoBytes.length);
				drawIcon(bitmapContainer, icon);
			}
		}
	}
	...
	private boolean commitChip(int start, int end, Editable editable) {
		...
		if (position != -1 && enoughToFilter() && end == getSelectionEnd() && !isPhoneQuery() && !isValidEmailAddress(editable.toString().substring(start, end).trim())) {
	...
	private boolean touchedWarningIcon(float x, float y, DrawableRecipientChip currentChip) {
		...
		if (currentChip != null) {
			...
			if (outOfDomainWarningBounds != null) {
				int chipLeftOffset = shouldPositionAvatarOnRight() ? getChipEnd(currentChip) : getChipStart(currentChip);
				float chipLeftPosition = this.getLayout().getPrimaryHorizontal(chipLeftOffset);
				float chipTopPosition = this.getLayout().getLineTop(this.getLayout().getLineForOffset(chipLeftOffset)) + getTotalPaddingTop();
				final RectF touchOutOfDomainWarning = new RectF(chipLeftPosition + outOfDomainWarningBounds.left, chipTopPosition + outOfDomainWarningBounds.top, chipLeftPosition + outOfDomainWarningBounds.right, chipTopPosition + outOfDomainWarningBounds.bottom);
	...
	private void showWarningDialog(String warningText) {
		...
		new AlertDialog.Builder(RecipientEditTextView.this.getContext()).setTitle(mWarningTitle).setOnDismissListener(new DialogInterface.OnDismissListener() {
			@Override
			public void onDismiss(DialogInterface dialog) {
				mCurrentWarningText = "";
			}
		}).setMessage(mCurrentWarningText).show();
	}
	...
	protected ListAdapter createAlternatesAdapter(DrawableRecipientChip chip) {
		return new RecipientAlternatesAdapter(getContext(), chip.getContactId(), chip.getDirectoryId(), chip.getLookupKey(), chip.getDataId(), getAdapter().getQueryType(), this, mDropdownChipLayouter, constructStateListDeleteDrawable(), getAdapter().getPermissionsCheckListener());
	}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		if (entry.getEntryType() == RecipientEntry.ENTRY_TYPE_PERMISSION_REQUEST) {
			if (mPermissionsRequestItemClickedListener != null) {
				mPermissionsRequestItemClickedListener.onPermissionsRequestItemClicked(this, entry.getPermissions());
			}
			return;
		}
	...
	private boolean isValidEmailAddress(String input) {
		return !TextUtils.isEmpty(input) && mValidator != null && mValidator.isValid(input);
	}
	...
	public void removeRecipientEntry(final RecipientEntry entry) {
		...
		for (final DrawableRecipientChip recipient : recips) {
			...
			if (existingEntry != null && existingEntry.isValid() && existingEntry.isSamePerson(entry)) {
