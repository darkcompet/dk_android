...
public class TheHost extends ViewModel implements TheClient.Listener {
	...
	private final ArrayMap<String, OwnTopic> allTopics = new ArrayMap<>();
	...
	public <M> M obtainModel(String topicId, String modelKey, Class<M> modelType) throws Exception {
		// Get or Create topic
		OwnTopic topic = allTopics.get(topicId);
		if (topic == null) {
			topic = new OwnTopic(topicId);
	 ...
	 */
	public void registerClient(String topicId, TheClient client) {
		// Get or Create topic
		OwnTopic topic = allTopics.get(topicId);
		if (topic == null) {
			topic = new OwnTopic(topicId);
	 ...
	 */
	public void unregisterClient(String topicId, TheClient client) {
		OwnTopic topic = allTopics.get(topicId);
	...
	public void onClientDisconnect(TheClient client) {
		for (int index = allTopics.size() - 1; index >= 0; --index) {
			OwnTopic topic = allTopics.valueAt(index);
