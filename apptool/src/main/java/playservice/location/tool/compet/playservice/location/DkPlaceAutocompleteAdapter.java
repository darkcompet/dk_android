...
public class DkPlaceAutocompleteAdapter extends ArrayAdapter<AutocompletePrediction> implements Filterable {
	...
	private ArrayList<AutocompletePrediction> mAutocompletePredict = new ArrayList<>();
	...
	@Override
	public int getCount() {
		return mAutocompletePredict.size();
	}

	@Override
	public AutocompletePrediction getItem(int pos) {
		return (pos >= 0 && pos < mAutocompletePredict.size()) ? mAutocompletePredict.get(pos) : null;
	}
	...
	public Filter getFilter() {
		return new Filter() {
			...
			protected FilterResults performFiltering(CharSequence constraint) {
				...
				if (constraint != null) {
					mAutocompletePredict.clear();
					mAutocompletePredict.addAll(getAutocomplete(constraint));
					results.values = mAutocompletePredict;
					results.count = mAutocompletePredict.size();
				}
				return results;
			}
