...
import tool.compet.chips.BaseRecipientAdapter.DirectoryListQuery;
import tool.compet.chips.BaseRecipientAdapter.DirectorySearchParams;
import tool.compet.chips.DropdownChipLayouter.AdapterType;
import tool.compet.chips.Queries.Query;
...
public class RecipientAlternatesAdapter extends CursorAdapter {
	...
	public static void getMatchingRecipients(Context context, BaseRecipientAdapter adapter, ArrayList<String> inAddresses, Account account, RecipientMatchCallback callback, ChipsUtil.PermissionsCheckListener permissionsCheckListener) {
		getMatchingRecipients(context, adapter, inAddresses, QUERY_TYPE_EMAIL, account, callback, permissionsCheckListener);
	}
	...
	public static void getMatchingRecipientsFromDirectoryQueries(Context context, Map<String, RecipientEntry> recipientEntries, Set<String> addresses, Account account, Set<String> matchesNotFound, RecipientMatchCallback callback, ChipsUtil.PermissionsCheckListener permissionsCheckListener) {
		getMatchingRecipientsFromDirectoryQueries(context, recipientEntries, addresses, account, matchesNotFound, Queries.EMAIL, callback, permissionsCheckListener);
	}

	private static void getMatchingRecipientsFromDirectoryQueries(Context context, Map<String, RecipientEntry> recipientEntries, Set<String> addresses, Account account, Set<String> matchesNotFound, Queries.Query query, RecipientMatchCallback callback, ChipsUtil.PermissionsCheckListener permissionsCheckListener) {
			...
			if (paramsList != null) {
				...
				for (String unresolvedAddress : unresolvedAddresses) {
					for (int i = 0; i < paramsList.size(); i++) {
						...
						try {
							directoryContactsCursor = doQuery(unresolvedAddress, 1 /* limit */, directoryId, account, context, query, permissionsCheckListener);
							if (directoryContactsCursor != null && directoryContactsCursor.getCount() != 0) {
								// We found the directory with at least one contact
								final Map<String, RecipientEntry> entries = processContactEntries(directoryContactsCursor, directoryId);
	...
	private static HashMap<String, RecipientEntry> processContactEntries(Cursor c, Long directoryId) {
		...
		if (c != null && c.moveToFirst()) {
			do {
				...
				final RecipientEntry newRecipientEntry = RecipientEntry.constructTopLevelEntry(c.getString(Queries.Query.NAME), c.getInt(Queries.Query.DISPLAY_NAME_SOURCE), c.getString(Queries.Query.DESTINATION), c.getInt(Queries.Query.DESTINATION_TYPE), c.getString(Queries.Query.DESTINATION_LABEL), c.getLong(Queries.Query.CONTACT_ID), directoryId, c.getLong(Queries.Query.DATA_ID), c.getString(Queries.Query.PHOTO_THUMBNAIL_URI), true, c.getString(Queries.Query.LOOKUP_KEY));
	...
	private static Cursor doQuery(CharSequence constraint, int limit, Long directoryId, Account account, Context context, Query query, ChipsUtil.PermissionsCheckListener permissionsCheckListener) {
		if (!ChipsUtil.hasPermissions(context, permissionsCheckListener)) {
			if (Log.isLoggable(TAG, Log.DEBUG)) {
		...
		final Uri.Builder builder = query.getContentFilterUri().buildUpon().appendPath(constraint.toString()).appendQueryParameter(ContactsContract.LIMIT_PARAM_KEY, String.valueOf(limit + BaseRecipientAdapter.ALLOWANCE_FOR_DUPLICATES));
		if (directoryId != null) {
		...
		return context.getContentResolver().query(builder.build(), query.getProjection(), null, null, null);
	}

	public RecipientAlternatesAdapter(Context context, long contactId, Long directoryId, String lookupKey, long currentId, int queryMode, OnCheckedItemChangedListener listener, DropdownChipLayouter dropdownChipLayouter, ChipsUtil.PermissionsCheckListener permissionsCheckListener) {
		this(context, contactId, directoryId, lookupKey, currentId, queryMode, listener, dropdownChipLayouter, null, permissionsCheckListener);
	}

	public RecipientAlternatesAdapter(Context context, long contactId, Long directoryId, String lookupKey, long currentId, int queryMode, OnCheckedItemChangedListener listener, DropdownChipLayouter dropdownChipLayouter, StateListDrawable deleteDrawable, ChipsUtil.PermissionsCheckListener permissionsCheckListener) {
		super(context, getCursorForConstruction(context, contactId, directoryId, lookupKey, queryMode, permissionsCheckListener), 0);
	...
	private static Cursor getCursorForConstruction(Context context, long contactId, Long directoryId, String lookupKey, int queryType, ChipsUtil.PermissionsCheckListener permissionsCheckListener) {
		final Uri uri;
		...
		if (queryType == QUERY_TYPE_EMAIL) {
			...
			if (directoryId == null || lookupKey == null) {
				...
				uri = Contacts.getLookupUri(contactId, lookupKey).buildUpon().appendPath(Contacts.Entity.CONTENT_DIRECTORY).appendQueryParameter(ContactsContract.DIRECTORY_PARAM_KEY, String.valueOf(directoryId)).build();
		...
		final String selection = new StringBuilder().append(projection[Queries.Query.CONTACT_ID]).append(" = ?").toString();
		final Cursor cursor;
		if (ChipsUtil.hasPermissions(context, permissionsCheckListener)) {
			cursor = context.getContentResolver().query(uri, projection, selection, new String[]{String.valueOf(contactId)}, null);
		}
		else {
	 ...
	 */
	// Visible for testing
	static Cursor removeUndesiredDestinations(final Cursor original, final String desiredMimeType, final String lookupKey) {
	...
	public RecipientEntry getRecipientEntry(int position) {
		...
		return RecipientEntry.constructTopLevelEntry(c.getString(Queries.Query.NAME), c.getInt(Queries.Query.DISPLAY_NAME_SOURCE), c.getString(Queries.Query.DESTINATION), c.getInt(Queries.Query.DESTINATION_TYPE), c.getString(Queries.Query.DESTINATION_LABEL), c.getLong(Queries.Query.CONTACT_ID), mDirectoryId, c.getLong(Queries.Query.DATA_ID), c.getString(Queries.Query.PHOTO_THUMBNAIL_URI), true, c.getString(Queries.Query.LOOKUP_KEY));
	}
	...
	public void bindView(View view, Context context, Cursor cursor) {
		...
		mDropdownChipLayouter.bindView(view, null, entry, position, AdapterType.RECIPIENT_ALTERNATES, null, mDeleteDrawable);
	}
