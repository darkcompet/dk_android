...
public class DropdownChipLayouter {
	 ...
	 */
	public enum AdapterType {BASE_RECIPIENT, RECIPIENT_ALTERNATES, SINGLE_RECIPIENT}
	...
	public DropdownChipLayouter(LayoutInflater inflater, Context context) {
		mInflater = inflater;
		mContext = context;
		mAutocompleteDividerMarginStart = context.getResources().getDimensionPixelOffset(R.dimen.chip_wrapper_start_padding);
	}
	...
	protected void bindDrawableToDeleteView(final StateListDrawable drawable, String recipient, ImageView view) {
		if (view == null) {
			return;
		}
		if (drawable == null) {
			...
			view.setContentDescription(res.getString(R.string.dropdown_delete_button_desc, recipient));
	...
	protected void bindIndicatorToView(@DrawableRes int indicatorIconId, String indicatorText, TextView view) {
		if (view != null) {
			if (indicatorText != null || indicatorIconId != 0) {
				...
				view.setCompoundDrawablesRelativeWithIntrinsicBounds(indicatorIcon, null, null, null);
			}
			else {
				view.setVisibility(View.GONE);
			}
	...
	protected CharSequence getDestinationType(RecipientEntry entry) {
		return mQuery.getTypeLabel(mContext.getResources(), entry.getDestinationType(), entry.getDestinationLabel()).toString().toUpperCase();
	}
	 ...
	 */
	protected @LayoutRes
	int getAlternateItemLayoutResId(AdapterType type) {
		switch (type) {
			case BASE_RECIPIENT:
				return R.layout.chips_autocomplete_recipient_dropdown_item;
			case RECIPIENT_ALTERNATES:
	 ...
	 */
	protected @IdRes
	int getDeleteResId() { return android.R.id.icon1; }
	 ...
	 */
	protected @IdRes
	int getPermissionRequestDismissResId() {
		...
		for (int i = 0; i < results.length; i++) {
			...
			if (!foundMatch) {
				...
				if (index != -1) {
					...
					ForegroundColorSpan highlightSpan = new ForegroundColorSpan(mContext.getResources().getColor(R.color.chips_dropdown_text_highlighted));
					styled.setSpan(highlightSpan, index, index + constraint.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
	...
	protected class ViewHolder {
		...
		public ViewHolder(View view) {
			...
			permissionRequestDismissView = (ImageView) view.findViewById(getPermissionRequestDismissResId());
		}
	}
}
